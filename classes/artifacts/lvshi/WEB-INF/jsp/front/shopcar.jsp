			...
			<div class="row">
				<div class="col-md-4 col-md-offset-8 ">
					...
					<form name="form2" id="registerFormcustomer" method="post" action="">
						...
						<c:forEach items="${addressList }" var="lists">
							<div class="form-group">
								<input type="radio"  value="${lists.id }" name="address" placeholder="">${lists.lxr },${lists.phone},${lists.xxdz}
							</div>
						</c:forEach>
						</c:if>
						<c:if test="${addressList==null||fn:length(addressList)==0 }">
							<a href="${ctx }/front/mineaddress.html">请先添加联系地址</a>
						</c:if>
					</form>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function qrzf(total) {
		...
		var params = {total:total,address:address};
